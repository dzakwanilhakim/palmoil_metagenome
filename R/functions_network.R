...
# ----------------------------------------------------------------------------
# Generic keystone analysis for ONE rank (genus or species) on ONE subset.
#   clr_sub : sample x taxon CLR matrix for this subset
#   meta    : metadata for those samples (id_sampel, timepoint, ...)
#   rank    : "genus" or "species"  -> used for filenames + titles
# Produces (prefixed by rank):
#   <rank>_network_hubs.jpg, <rank>_heatmap.png,
#   keystone_<rank>_active10.csv (topology x DA intersection),
#   volcano_<rank>_T0_vs_T1.png (+csv), dumbbell_<rank>.png
# Returns list(written, keystones).
# ----------------------------------------------------------------------------
run_rank_keystone <- function(clr_sub, meta, rank, leaf, tag,
                              r_thresh = R_THRESH) {
  dir.create(leaf, recursive = TRUE, showWarnings = FALSE)
  label <- paste0(tag, " | ", rank)
  g <- build_network(clr_sub, label, r_thresh = r_thresh)

  if (is.null(g)) {
    readr::write_csv(
      tibble::tibble(note = paste0("skipped n<", MIN_N_NET, ": ", label)),
      file.path(leaf, paste0(rank, "_network_skipped.csv")))
    return(list(written = character(0), keystones = tibble::tibble()))
  }

  written <- character(0)

  # hub network (green=+ / red=- edges)
  w <- plot_network_hubs(g, paste0(tag, " | ", rank, " hub network"),
                          file.path(leaf, paste0(rank, "_network_hubs.jpg")))
  if (!is.na(w)) written <- c(written, w)

  # correlation heatmap (top hubs)
  w <- plot_corr_heatmap(g, paste0(tag, " | ", rank, " correlation heatmap"),
                          file.path(leaf, paste0(rank, "_heatmap.png")))
  if (!is.na(w)) written <- c(written, w)

  # Active keystones: topology (degree) x DA (Welch t-test, raw p<0.05)
  ks <- active_keystones(g, clr_sub, meta)
  # standardize the taxon column name to "taxon" for downstream plotting
  if (nrow(ks) > 0 && "genus" %in% names(ks))
    ks <- dplyr::rename(ks, taxon = genus)
  readr::write_csv(ks, file.path(leaf, paste0("keystone_", rank, "_active10.csv")))

  # volcano (Welch t-test on this rank's CLR)
  written <- c(written,
    volcano_for_subset(clr_sub, meta, rank, leaf, tag))

  # dumbbell of THIS rank's Top-10 keystones (own network)
  if (nrow(ks) > 0) {
    w <- plot_keystone_dumbbell(clr_sub, meta, ks, rank,
           paste0(tag, " | ", rank, " keystone dumbbell"),
           file.path(leaf, paste0("dumbbell_", rank, ".png")))
    if (!is.na(w)) written <- c(written, w)
  }

  list(written = written, keystones = ks)
}

# Dispatcher: run genus track, then species track, on the same subset samples.
.run_network_subset <- function(clr_sub_genus, label, leaf, tag,
                                meta_full = NULL,
  ...
  # ---- GENUS track --------------------------------------------------------
  resG <- run_rank_keystone(clr_sub_genus, meta_full, "genus", leaf, tag,
                            r_thresh = r_thresh)
...
species_clr_for_ids <- function(species_long, ids, marker, stage) {
  if (is.null(species_long) || nrow(species_long) == 0) {
    message("    [sp_clr] species_long is NULL/empty"); return(NULL)
  }
  d <- species_long |>
  ...
  if (nrow(d) == 0) {
    message("    [sp_clr] 0 rows after filter: marker=", marker, " stage=", stage,
            " ids=", length(ids),
            " | species_long markers=",
            paste(unique(species_long$marker), collapse=","),
            " stages=", paste(unique(species_long$stage), collapse=","))
    return(NULL)
  }
  wide <- d |>
  ...
  if (nrow(mat) < 2 || ncol(mat) < 2) {
    message("    [sp_clr] too small: ", nrow(mat), "x", ncol(mat)); return(NULL)
  }
...
.dumbbell_plot <- function(long_df, title, out_path, rank_label = "Genus") {
  ...
  has_deg <- "degree" %in% names(long_df) && any(!is.na(long_df$degree))
  ...
  if (has_deg) {
    p <- p + ggplot2::geom_point(
      ggplot2::aes(fill = timepoint, size = degree),
      shape = 21, colour = "grey30", stroke = .3) +
      ggplot2::scale_size_continuous(range = c(2.5, 9), name = "Node degree")
  } else {
    p <- p + ggplot2::geom_point(
      ggplot2::aes(fill = timepoint), shape = 21, size = 4,
      colour = "grey30", stroke = .3)
  }
  p <- p +
...
# Generic keystone dumbbell: plots the keystone taxa's own CLR across timepoints.
plot_keystone_dumbbell <- function(clr_mat, meta, ks, rank, title, out_path) {
  if (nrow(ks) == 0 || is.null(clr_mat)) return(NA_character_)
  taxa <- intersect(ks$taxon, colnames(clr_mat))
  if (length(taxa) == 0) return(NA_character_)
  df <- as.data.frame(clr_mat[, taxa, drop = FALSE])
  df$id_sampel <- rownames(clr_mat)
  long <- dplyr::inner_join(df,
    ...
    # attach node degree (dot size = network connectivity)
    dplyr::left_join(dplyr::select(ks, taxon, degree), by = "taxon")
  lab <- if (rank == "species") "Species" else "Genus"
  .dumbbell_plot(long, title, out_path, rank_label = lab)
}
